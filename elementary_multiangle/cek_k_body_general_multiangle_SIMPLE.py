...
#def get_expectation(p, n_gamma, shots=1*512):  
def get_expectation(p, n_gamma):  
    """
    ...
    def execute_circ(theta):
        ...
        #qc=create_qaoa_circ_from_terms_multiparams_multilayers_simple(qx, nqubits, theta)
        qc=create_qaoa_circ_from_terms_mparams_mlayers_simple(qx, nqubits, n_gamma, theta)
...
#H4='s0 + 2*s1 + 3*s2 + 2*s0*s1 + 3*s1*s2 + 5*s1*s2*s3'
#H4='s0 + s1 + s2 + s0*s1 + s1*s2 + s1*s2*s3 + s0*s1*s2*s3 - 1'
#H4='s0 + 2*s1 + 3*s2 + 5*s0*s1 + 7*s1*s2 + 11*s1*s2*s3 + 13*s0*s1*s2*s3'
H4='s0*s1 + 2*s0*s2 + 3*s0*s3 + 5*s1*s2 + 7*s1*s3+ 11*s2*s3-3'
#H4='s0*s1 + s0*s2 + s0*s3 + s1*s2 + s1*s3+ s2*s3'
##################
vObj=[]
vShots=[]
vCorrect=[]
...
if __name__ == '__main__':
    ...
    ###############################
    # ///// Number of Layers /////
    NLAYER= 1 #int(2*NQ*NQ) #NQ*NQ #NQ #int(1*NQ) #NQ*4 #NQ*5 #1 #10 #upto 10 layer is ok for orde-68
    ###############################
    ...
    expectation=get_expectation(NLAYER, n_gamma)
    ...
    #qc_res=create_qaoa_circ_from_terms_multiparams_multilayers_simple(qx, nqubits, res.x)
    qc_res=create_qaoa_circ_from_terms_mparams_mlayers_simple(qx, nqubits, n_gamma, res.x)
    #--    
